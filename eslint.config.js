...
export default tseslint.config(
  // Never lint build output, deps, the bun cache, or generated route trees.
  { ignores: ['dist', 'node_modules', '.bun', '.output', '.vite-out', 'build', '**/*.gen.ts'] },
  {
    ...
    rules: {
      ...
      'react-hooks/exhaustive-deps': 'warn', // Explicitly pin as warning to prevent future escalation
      
      // === DISABLE define-before-use entirely to avoid React false positives ===
      ...
      // Enable no-undef only for JS files - TS handles this better
      'no-undef': 'off', // Will be overridden for JS files below
      'no-func-assign': 'error',      // Function reassignment causes TypeError
      'no-dupe-keys': 'error',        // Duplicate object keys (silent data corruption)
      'use-isnan': 'error',           // Incorrect NaN checks
  ...
  // Enable no-undef for JS files only (TS files handled by TypeScript compiler)
  {
    files: ['**/*.{js,jsx}'],
    languageOptions: {
      globals: {
    ...
    rules: {
      'no-undef': 'error', // Enable undefined variable detection for JS files
    },
  },
  // Override for Node.js scripts - provide Node globals
  {
    files: ['scripts/**/*.js'],
    languageOptions: {
      globals: {
    ...
    rules: {
      'no-undef': 'error', // Keep undef detection but with Node globals available
    },
  },
  ...
  // TanStack Start route + layout modules export `Route`/`ServerRoute` (non-component
  // objects) alongside their page component — expected, so react-refresh doesn't apply.
  {
    files: ['src/routes/**/*.{ts,tsx}', 'src/layouts/**/*.{ts,tsx}'],
    rules: {
      'react-refresh/only-export-components': 'off',
    },
  },
